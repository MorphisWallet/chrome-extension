  ...
  return useMemo(() => {
    switch (type) {
      ...
      case ExplorerLinkType.object:
        return objectID && getObjectUrl(objectID, selectedApiEnv, customRPCUrl)
      case ExplorerLinkType.transaction:
        return (
          transactionID &&
          getTransactionUrl(transactionID, selectedApiEnv, customRPCUrl)
        )
      case ExplorerLinkType.validator:
        return (
          validator && getValidatorUrl(validator, selectedApiEnv, customRPCUrl)
        )
    }
  }, [
    type,
