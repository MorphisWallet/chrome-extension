...
function getAmount(
  ...
  const paySuiData = getPaySuiTransaction(txnData) ?? getPayTransaction(txnData)

  const amountByRecipient =
    paySuiData?.recipients.reduce((acc, value, index) => {
      return {
        ...acc,
        [value]: paySuiData.amounts[index] + (value in acc ? acc[value] : 0),
      }
...
>(
  ...
  async (
    ...
    const transactions: GetTxnDigestsResponse =
      await api.instance.fullNode.getTransactionsForAddress(address, true)

    if (!transactions || !transactions.length) {
      return []
    }

    const txEffs = await api.instance.fullNode.getTransactionWithEffectsBatch(
    ...
    const txResults = txEffs.map((txEff) => {
      ...
      const recipient = transferSui?.recipient ?? txTransferObject?.recipient
    ...
    const getObjectBatch = await api.instance.fullNode.getObjectBatch(objectIDs)
    const txObjects = getObjectBatch.filter(({ status }) => status === 'Exists')
