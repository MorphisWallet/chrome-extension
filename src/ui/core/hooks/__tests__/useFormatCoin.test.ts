...
describe('formatBalance', () => {
  ...
  it('formats integer amounts correctly', () => {
    ...
    expect(formatBalance(toMist('12345'), SUI_DECIMALS)).toEqual('12.345 K')
    // Millions:
    expect(formatBalance(toMist('1234000'), SUI_DECIMALS)).toEqual('1.234 M')
    // Billions:
    expect(formatBalance(toMist('1234000000'), SUI_DECIMALS)).toEqual('1.234 B')
  })

  it('formats integer amounts with full CoinFormat', () => {
    expect(formatBalance(toMist('1'), SUI_DECIMALS, CoinFormat.FULL)).toEqual(
      '1'
    )
    expect(
